...
public class PostCheckTurnRouteTest {
    ...
    private Game game;
    private final PostCheckTurnRoute CuT = new PostCheckTurnRoute(gameCenter);
    ...
    @Test
    public void test_player_has_turn_when_true() {

        //check who's turn it is
        int playerWithTurn = 1;
        if(game.isMyTurn(1)==false){playerWithTurn = 2;}

        final Response response = mock(Response.class);
        Request myreq = request_ch;
        if(playerWithTurn==2){
            myreq = request_op;
        }
        final Object resultobject = CuT.handle(myreq,response);
        //Verify result is not null
        assertNotNull(resultobject);
        boolean  result = (Boolean) resultobject;
        assertTrue(result);
    ...
    @Test
    public void test_player_has_turn_when_false() {
