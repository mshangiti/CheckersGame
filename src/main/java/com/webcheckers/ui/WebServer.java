...
public class WebServer{
  ...
  /**
   * The URL pattern to request the root.
   */
   ...
   * The URL pattern to request the Result page.
   */
  public static final String RESULT_URL = "/result";

  /**

   * The URL pattern to request the a game resign
   */
  public static final String RESIGN_URL = "/resign";

  /*
   * The URL pattern to submit a move
   */
  ...
  /**
   *  The URL pattern to check the current turn
   */
  public static final String CHECK_TURN_URL = "/checkTurn";

  /**
   *  The URL pattern to backup last move
   */
   ...
   */
  public WebServer(final GameCenter gameCenter, final TemplateEngine templateEngine) {
  ...
  public void initialize() {
    ...
    //setting routes
    get(ROOT_URL, new GetSignInRoute(gameCenter), templateEngine);
    get(SIGNIN_URL, new GetSignInRoute(gameCenter), templateEngine);
    post(SIGNIN_URL, new PostSignInRoute(gameCenter), templateEngine);
    get(SIGNOUT_URL, new GetSignOutRoute(gameCenter), templateEngine);
    get(HOME_URL, new HomeController(gameCenter), templateEngine);
    post(HOME_URL, new PostHomeRoute(gameCenter), templateEngine);
    get(GAME_URL, new GameRoute(gameCenter), templateEngine);
    post(GAME_URL, new GameRoute(gameCenter), templateEngine);
    post(SUBMIT_URL, new SubmitRoute(gameCenter), templateEngine);
    post(VALIDATE_URL, new ValidateRoute(gameCenter), JsonUtils.json());
    post(CHECK_TURN_URL, new PostCheckTurnRoute(gameCenter), JsonUtils.json());
    get(RESULT_URL, new GetResultRoute(gameCenter), templateEngine);
    post(RESIGN_URL, new PostResignRoute(gameCenter), JsonUtils.json());
    post(BACKUP_MOVE, new PostBackupMoveRoute(gameCenter), JsonUtils.json());
  }//end of function
}